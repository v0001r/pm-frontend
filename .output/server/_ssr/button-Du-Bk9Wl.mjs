...
import { C as require_jsx_runtime, v as Slot } from "../_libs/@radix-ui/react-accordion+[...].mjs";
import { t as axios } from "../_libs/axios+[...].mjs";
import { n as clsx, t as cva } from "../_libs/class-variance-authority+clsx.mjs";
import { t as twMerge } from "../_libs/tailwind-merge.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/button-Du-Bk9Wl.js
var import_react = /* @__PURE__ */ __toESM(require_react());
var import_jsx_runtime = require_jsx_runtime();
var SESSION_EXPIRED_EVENT = "auth:session-expired";
var GUEST_AUTH_PATHS = /* @__PURE__ */ new Set([
	"/",
	"/forgot-password",
	"/reset-password",
	"/activate"
...
function buildLoginSearch(options) {
	const search = {};
	if (options?.expired) search.expired = "1";
...
var sessionExpiryInFlight = false;
function isSessionExpiryInFlight() {
	return sessionExpiryInFlight;
}
function notifySessionExpired(returnTo) {
	if (typeof window === "undefined" || sessionExpiryInFlight) return;
	sessionExpiryInFlight = true;
	window.dispatchEvent(new CustomEvent(SESSION_EXPIRED_EVENT, { detail: { returnTo: sanitizeReturnTo(returnTo) ?? getCurrentReturnTo() } }));
...
function getApiFieldErrors(error) {
	...
	const data = error.response?.data;
	if (!data?.fieldErrors || typeof data.fieldErrors !== "object") return {};
	return Object.fromEntries(Object.entries(data.fieldErrors).filter((entry) => typeof entry[1] === "string"));
}
...
var isStaff = (role) => role === "Admin" || role === "Staff";
...
async function validateActivationToken(token) {
	const { data } = await api.get("/auth/activate/validate", { params: { token } });
	return data.data;
}
...
var buttonVariants = cva("inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium cursor-pointer transition-all duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40 focus-visible:ring-offset-2 focus-visible:ring-offset-background disabled:pointer-events-none disabled:opacity-50 disabled:cursor-not-allowed [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0", {
	variants: {
		variant: {
			default: "bg-primary text-primary-foreground shadow-sm hover:bg-primary/90 active:scale-[0.98]",
			destructive: "bg-destructive text-destructive-foreground shadow-sm hover:bg-destructive/90 active:scale-[0.98]",
			outline: "border border-border bg-surface text-foreground shadow-sm hover:bg-accent hover:text-accent-foreground active:scale-[0.98]",
...
//#endregion
export { isStaff as _, api as a, useAuth as b, cn as c, getApiFieldErrors as d, getCurrentReturnTo as f, isSessionExpiryInFlight as g, isGuestAuthPath as h, activateAccount as i, forgotPassword as l, isAdmin as m, Button as n, buildLoginSearch as o, homeFor as p, SESSION_EXPIRED_EVENT as r, buttonVariants as s, AuthProvider as t, getApiErrorMessage as u, resetPassword as v, validateActivationToken as x, sanitizeReturnTo as y };
